import { generateDeepTaxonomy, getFoaf, getTimblAndFoaf, getRdfs } from 'deep-taxonomy-benchmark';
// @ts-expect-error
import { Reasoner, Parser } from 'n3';
import { rdfs, incremental, quadsToFacts } from 'hylar-core';
...
async function deepTaxonomy(extended = false) {
    ...
    if (process.env.EYE) {
        for (let i = 1; i <= 6; i++) {
            ...
            await queryOnce(swipl, 'main', ['--nope', '--quiet', './data.n3', '--pass-only-new']);
